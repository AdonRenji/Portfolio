...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-24 relative overflow-hidden border-t border-zinc-900 bg-[#040114]">
      {/* Background glow */}
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div
                ...
                className="group cursor-pointer flex flex-col rounded-3xl glass-panel border border-white/5 p-6 hover:border-indigo-500/30 transition-all duration-300 hover:shadow-[0_10px_30px_rgba(99,102,241,0.05)] hover:-translate-y-2 h-full"
              >
                <div className="flex items-start justify-between mb-6">
                  <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 group-hover:bg-indigo-500 group-hover:text-white transition-colors duration-300 shadow-inner">
                ...
                <h3 className="text-xl font-bold text-white group-hover:text-indigo-300 transition-colors duration-200 mb-3">
                  {project.title}
                </h3>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-6">
                  {project.tech.map((t) => (
                    <span key={t} className="text-xs font-mono text-zinc-500 bg-white/5 px-2 py-1 rounded">
                      {t}
                    </span>
                  ))}
                </div>
        ...
        {selectedProject && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-x-4 bottom-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 z-[101] max-w-2xl w-full max-h-[85vh] overflow-y-auto rounded-3xl glass-panel border border-indigo-500/20 shadow-[0_20px_60px_rgba(0,0,0,0.5)] p-6 md:p-10 bg-[#0a0a0a]/95"
            >
              <button
                onClick={() => setSelectedProject(null)}
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap gap-2">
                  {selectedProject.tech.map((t) => (
                    <span key={t} className="text-xs font-mono font-medium text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1.5 rounded-md">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  <h4 className="text-lg font-semibold text-zinc-200">Key Features & Highlights</h4>
                  <ul className="space-y-3">
                    {selectedProject.description.map((point, idx) => (
                      <li key={idx} className="flex items-start gap-3 text-zinc-300 font-light leading-relaxed">
                  ...
                  <div className="flex items-center gap-4 pt-4">
                    {selectedProject.github && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white font-medium transition-colors"
                      >
                        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                          <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
                        </svg>
